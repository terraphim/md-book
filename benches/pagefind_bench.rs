...
async fn create_benchmark_site(
    temp_dir: &TempDir,
    page_count: usize,
            ...
            |b, &(page_count, words_per_page)| {
                b.iter(|| {
                    rt.block_on(async {
                        let temp_dir = TempDir::new().expect("Failed to create temp dir");
                        let site_path =
                            create_benchmark_site(&temp_dir, page_count, words_per_page)
                                .await
                                .expect("Failed to create site");

                        let builder = PagefindBuilder::new(black_box(site_path))
                            .await
...
fn bench_error_handling(c: &mut Criterion) {
    ...
    group.bench_function("invalid_path", |b| {
        b.iter(|| {
            rt.block_on(async {
                let invalid_path =
                    PathBuf::from("/absolutely/nonexistent/path/that/should/never/exist");
                ...
                b.iter(|| {
                    rt.block_on(async {
                        ...
                        let tasks = (0..concurrency).map(|_| {
                            ...
                            async move { PagefindBuilder::new(black_box(site_path)).await }
                        });
